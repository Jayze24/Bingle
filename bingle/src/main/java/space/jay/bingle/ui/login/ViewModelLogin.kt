...
import space.jay.bingle.ui.dialog.DialogManager
...
class ViewModelLogin(application: Application) : AndroidViewModel(application) {
    ...
    fun sendVersionServerToLiveData(binding: ActivityLoginBinding, boxVersion: BoxVersion) {
        binding.isSigned = true
        boxVersion.sendVersionServerToLiveData(mLoginLiveData)
    }

    fun setObserve(activity: ActivityLogin, binding: ActivityLoginBinding, boxVersion: BoxVersion) {
        mLoginLiveData.observe(activity, Observer { handleMessage(activity, it, binding, boxVersion) })
    }

    private fun handleMessage(activity: ActivityLogin, message: LiveDataObject, binding: ActivityLoginBinding, boxVersion: BoxVersion) {
        when (message.result) {
            Constants.Server.CONNECTION_SUCCESS -> {
                when (message.event) {
                    //1. 인증 성공 후 버전 정보 요청
                    Constants.Event.LOGIN_GOOGLE -> sendVersionServerToLiveData(binding, boxVersion)

                    //2. 서버에서 버전 정보를 가져와 이곳을 호출하면 버전 체크를 함
                    Constants.Event.VERSION -> {
                        when (message.message) {
                            boxVersion.getVersionFromServer().alert1 -> {
                                //필수 업데이트
                                DialogManager().show(
                                    activity,
                                    null,
                            ...
                            boxVersion.getVersionFromServer().alert2 -> {
                                //일반 업데이트
